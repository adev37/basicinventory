...
export const createGoodsReceipt = async (req, res) => {
  try {
    const { purchaseOrder, receivedItems, remarks, createdBy } = req.body;
    ...
    // 2. Check how much has already been received (sum up all GRNs for this PO)
    ...
    // 3. Validate and update stock only for valid items
    for (const r of receivedItems) {
      ...
      await increaseStock(r.item, r.receivedQty);
    }
